<section id="users-reviews" class="pb-10 sm:pb-16 md:pb-25 lg:pb-50">
    ...
    <div class="container-base" x-data="homeReviewsSwiper(@js($serviceReviews ?? []))" x-init="$nextTick(() => initSwiper())">
        <div class="xl:space-y-15">
            <h2 class="text-center lg:text-left">Отзывы пользователей</h2>

            <div class="swiper reviews-swiper overflow-hidden mt-8 xs:mt-15">
                        ...
                        <div class="swiper-slide h-auto! bg-brand-gray-light-2 rounded-brand-base xs:rounded-3xl lg:rounded-brand-3xl p-4 sm:p-7 lg:p-10">
                            <div class="h-full flex flex-col justify-between space-y-3 lg:space-y-5">
                                ...
                                <div class="text_2 pt-2 sm:pt-0">
                                    ...
                                    <p class="text-brand-gray-dark" x-text="review.authRole"></p>
                                </div>
                            </div>
                        </div>
                    </template>
            ...
            <div class="flex-between flex-col xs:flex-row gap-4 mt-5">
                @auth
                    <button type="button" class="button_3 order-2 xs:order-1" @click="$store.reviewModalOpen = true; $store.reviewModalOpenCount = $store.reviewModalOpenCount + 1">
                        Оставить отзыв
                    </button>
                @else
                    <button type="button" class="button_3 order-2 xs:order-1" @click="authModalOpen = true; authModalMode = 'login'">
                        Оставить отзыв
                    </button>
                @endauth
                <div class="flex-end gap-3 lg:gap-6.5 order-1 xs:order-2">
                    <button type="button" class="reviews-swiper-prev hover:text-brand-blue disabled:text-brand-gray hover:bg-brand-gray-light-2 rounded smooth p-2 disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!swiper || isBeginning" @click="swiper && swiper.slideTo(Math.max(swiper.activeIndex - 1, 0))" aria-label="Предыдущий отзыв">
                        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" class="size-10 lg:size-12">
                            <path d="M38 24H10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M24 38L10 24L24 10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </button>
                    <button type="button" class="reviews-swiper-next hover:text-brand-blue disabled:text-brand-gray hover:bg-brand-gray-light-2 rounded smooth p-2 disabled:opacity-50 disabled:cursor-not-allowed" :disabled="!swiper || isEnd" @click="swiper && swiper.slideTo(swiper.activeIndex + 1)" aria-label="Следующий отзыв">
                        <svg width="48" height="48" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" class="size-10 lg:size-12 rotate-180">
                            <path d="M38 24H10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                            <path d="M24 38L10 24L24 10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                        </svg>
                    </button>
