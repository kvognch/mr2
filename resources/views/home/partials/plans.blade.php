<section id="prices" class="pb-10 sm:pb-16 md:pb-25 lg:pb-32 3xl:pb-50">
    <div class="container-base space-y-10 3xl:space-y-15">
        <div class="space-y-2">
            <h2>{{ $settings['plans']['title'] }}</h2>
            <p class="text_2 max-w-5xl text-brand-gray-dark">{{ $settings['plans']['description'] }}</p>
        </div>

        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 md:gap-6 3xl:gap-10">
            @foreach ($settings['plans']['items'] as $item)
                <div class="space-y-5 lg:space-y-6 3xl:space-y-10 bg-brand-gray-light-2 rounded-brand-base 3xl:rounded-brand-3xl py-6 px-5 lg:px-6 3xl:p-10">
